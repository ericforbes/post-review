...
class TestGitHubService(unittest.TestCase):
    ...
    @mock.patch("postreview.GitHubService.GitHubService._generate_fingerprint")
    def test_github_fetch_token(self, fetch_fingerprint, fetch_credentials, mock_request):
        username = "frank"
        pw = "bigg13123"
        ...
        fetch_fingerprint.side_effect = ['0.123456789']
        local = "NewFeatureBranch"
        remote = "master"
        service = GitHubService(local, remote, 'git@github.com:ericforbes/post-review.git', 'ericforbes', 'post-review', None)
        ...
        self.assertEqual(kwargs.get('url'), 'https://api.github.com/authorizations/clients/%s/%s' % (GitHubService.CLIENT_ID, '0.123456789'))
